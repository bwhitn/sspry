...
- artifact root:
  - `/root/pertest/results/sspry_full_corpus_20260318_r4`
- corpus:
  - files: `260,278`
- stop point:
  - `238,785 / 260,278` (`91.74%`)
  - explicit stop after enough tail data
- corrected memory read:
  - plain process RSS is misleading here because the store is heavily mmap/file-backed
  - current live server split near the stop point:
    - `RssAnon ~= 2.26 GiB`
    - `RssFile ~= 17.37 GiB`
  - system memory pressure proxy:
    - `Active(anon) + Inactive(anon) ~= 9.90 GiB`
  - operational conclusion:
    - use anon metrics for memory-budget decisions
    - use file-backed resident + dirty/writeback for IO-pressure decisions
    - do not use plain RSS alone as the “server memory” number
- late-run throughput:
  - average files/minute had fallen into the `700s`
  - latest interval at the stop point:
    - `302.21 files/min`

Late-run slowdown read from that run:
- backpressure was not the cause:
  - `index_backpressure_events_total = 0`
  - pressure-publish during active index was disabled on that baseline
- the late-run climbers were:
  1. `compact_df_counts`
  2. `append_sidecars`
  3. `classify_df_lookup`
- current cumulative totals near the stop point:
  - `store_compact_df_counts_us = 7,684,791,804`
  - `store_append_sidecars_us = 5,536,962,324`
  - `store_classify_df_lookup_us = 4,496,412,047`
- interpretation:
  - the current slowdown is primarily writeback / compaction / sidecar-build pressure
  - not anonymous-memory exhaustion

Current implementation pass in progress:
- grouped local DF segment scans for sorted gram lookups landed locally
- idea:
  - keep the existing sparse fence index
  - group sorted grams by fence bucket
  - scan each bounded segment range once instead of binary-searching each gram independently
- rationale:
  - target `classify_df_lookup` locality directly
  - no correctness change
  - no large new resident structure
